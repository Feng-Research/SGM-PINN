...
from modulus.dataset import (
    ...
    No_SPADE_GraphImportanceSampledPointwiseIterableDataset,
)
...
class PointwiseConstraint(Constraint):
    ...
    def save_batch(self, filename):
        ...
        # testImportance = None
        # try:
        #     testImportance = self.dataset.save_batch_function(invar)
        # except:
        #     pass
        # if testImportance is not None:
        #     tests = ['continuity','momentum_x','momentum_y']
        #     for k,i in enumerate(tests):
        #         assert np.all(np.isclose(testImportance[:,k],save_var[i][:,0]))
        ##########
        ...
        if fixed_dataset:
            ...
            if importance_measure is None:
                ...
                if not SPADE_vars:
                    ...
                    dataset = No_SPADE_GraphImportanceSampledPointwiseIterableDataset(
                        invar=invar,
                        outvar=outvar,
                        batch_size=batch_size,
                        importance_measure=importance_measure,
                        ...
                        epochs_resample=epochs_resample, 
